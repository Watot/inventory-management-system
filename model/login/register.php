<?php
	...
	if(isset($_POST['registerUsername'])){
		...
		if(!empty($registerFullName) && !empty($registerUsername) && !empty($registerPassword1) && !empty($registerPassword2)){
			...
			$usernameCheckingSql = 'SELECT * FROM employee WHERE username = :username';
			...
			if($usernameCheckingStatement->rowCount() > 0){
				...
				if($registerPassword1 !== $registerPassword2){
					...
					if (!isStrongPassword($registerPassword1)) {
						echo '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">&times;</button>Your password should contain at least 8 characters, including at least one uppercase letter, one lowercase letter, one number, and one special character.</div>';
						exit();
					}
					...
					$insertemployeeSql = 'INSERT INTO employee(fullName, username, password) VALUES(:fullName, :username, :password)';
					$insertemployeeStatement = $conn->prepare($insertemployeeSql);
					$insertemployeeStatement->execute(['fullName' => $registerFullName, 'username' => $registerUsername, 'password' => $hashedPassword]);
